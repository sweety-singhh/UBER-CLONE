...
async function getFare(pickup, destination) {
    ...
    const pickupCor= await mapService.getAddressCoordinate(pickup);
    // console.log("pickupCor: " + pickupCor);
    const destinationCor= await mapService.getAddressCoordinate(destination);
    // console.log("destinationCor: " + destinationCor);
    const distanceTime = await mapService.getDistanceTime(pickupCor, destinationCor);
    const baseFare = {
        auto: 30,
        car: 50,
        moto: 20
